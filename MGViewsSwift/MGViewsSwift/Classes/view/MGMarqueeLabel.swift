...
open class MGMarqueeLabel: UILabel, CAAnimationDelegate {
    ...
    /**
     Defines the direction and method in which the `MGMarqueeLabel` instance scrolls.
     `MGMarqueeLabel` supports six default types of scrolling: `Left`, `LeftRight`, `Right`, `RightLeft`, `Continuous`, and `ContinuousReverse`.
    ...
    /**
     ...
     - Attention: Use of the `scrollSequence` property requires understanding of how MGMarqueeLabel works for effective
    ...
    /**
     A boolean property that sets whether the `MGMarqueeLabel` should behave like a normal `UILabel`.

     When set to `true` the `MGMarqueeLabel` will behave and look like a normal `UILabel`, and  will not begin any scrolling animations.
     Changes to this property take effect immediately, removing any in-flight animation as well as any edge fade. Note that `MGMarqueeLabel`
    ...
    /**
     A boolean property that sets whether the `MGMarqueeLabel` should hold (prevent) automatic label scrolling.

     When set to `true`, `MGMarqueeLabel` will not automatically scroll even its text is larger than the specified frame,
     although the specified edge fades will remain.

     To set `MGMarqueeLabel` to act like a normal UILabel, use the `labelize` property.
    ...
    /**
     A boolean property that sets whether the `MGMarqueeLabel` should only begin a scroll when tapped.

     If this property is set to `true`, the `MGMarqueeLabel` will only begin a scroll animation cycle when tapped. The label will
    ...
    @IBInspectable open var tapToScroll: Bool = false {
        didSet {
            if tapToScroll != oldValue {
                if tapToScroll {
                    let tapRecognizer = UITapGestureRecognizer(target: self, action: #selector(MGMarqueeLabel.labelWasTapped(_:)))
    ...
    /**
     The `MGMarqueeLabel` scrolling speed may be defined by one of two ways:
    ...
    /**
     Defines the speed of the `MGMarqueeLabel` scrolling animation.
    ...
    /**
     ...
     This propery sets the size (in points) of the view edge transparency fades on the left and right edges of a `MGMarqueeLabel`. The
    ...
    /**
     Convenience method to restart all `MGMarqueeLabel` instances that have the specified view controller in their next responder chain.

     - Parameter controller: The view controller for which to restart all `MGMarqueeLabel` instances.

     - Warning: View controllers that appear with animation (such as from underneath a modal-style controller) can cause some `MGMarqueeLabel` text
    ...
    class func restartLabelsOfController(_ controller: UIViewController) {
        MGMarqueeLabel.notifyController(controller, message: .Restart)
    }
    ...
    class func controllerViewWillAppear(_ controller: UIViewController) {
        MGMarqueeLabel.restartLabelsOfController(controller)
    }
    ...
    /**
     Labelizes all `MGMarqueeLabel` instances that have the specified view controller in their next responder chain.

     The `labelize` property of all recognized `MGMarqueeLabel` instances will be set to `true`.

     - Parameter controller: The view controller for which all `MGMarqueeLabel` instances should be labelized.
     - SeeAlso: labelize
     */
    class func controllerLabelsLabelize(_ controller: UIViewController) {
        MGMarqueeLabel.notifyController(controller, message: .Labelize)
    }

    /**
     De-labelizes all `MGMarqueeLabel` instances that have the specified view controller in their next responder chain.

     The `labelize` property of all recognized `MGMarqueeLabel` instances will be set to `false`.

     - Parameter controller: The view controller for which all `MGMarqueeLabel` instances should be de-labelized.
     - SeeAlso: labelize
     */
    class func controllerLabelsAnimate(_ controller: UIViewController) {
        MGMarqueeLabel.notifyController(controller, message: .Animate)
    }
    ...
    /**
     Returns a newly initialized `MGMarqueeLabel` instance with the specified scroll rate and edge transparency fade length.
     ...
     - Parameter fadeLength: A length of transparency fade at the left and right edges of the `MGMarqueeLabel` instance's frame.
     - Returns: An initialized `MGMarqueeLabel` object or nil if the object couldn't be created.
     - SeeAlso: fadeLength
     */
    ...
    /**
     Returns a newly initialized `MGMarqueeLabel` instance.
    ...
    private func setup() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(MGMarqueeLabel.restartForViewController(_:)), name: NSNotification.Name(rawValue: MarqueeKeys.Restart.rawValue), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(MGMarqueeLabel.labelizeForController(_:)), name: NSNotification.Name(rawValue: MarqueeKeys.Labelize.rawValue), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(MGMarqueeLabel.animateForController(_:)), name: NSNotification.Name(rawValue: MarqueeKeys.Animate.rawValue), object: nil)
        // UIApplication state notifications
        
        NotificationCenter.default.addObserver(self, selector: #selector(MGMarqueeLabel.restartLabel), name: UIApplication.didBecomeActiveNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(MGMarqueeLabel.shutdownLabel), name: UIApplication.didEnterBackgroundNotification, object: nil)
    }
    ...
    //
    // MARK: - MGMarqueeLabel Heavy Lifting
    //
    ...
    private func timingFunctionForAnimationCurve(_ curve: UIView.AnimationCurve) -> CAMediaTimingFunction {
        ...
        return CAMediaTimingFunction.init(name: timingFunction)
    }
    ...
    override open var numberOfLines: Int {
        ...
        set {
            // By the nature of MGMarqueeLabel, this is 1
            super.numberOfLines = 1
        }
    }
    ...
    override open var adjustsFontSizeToFitWidth: Bool {
        ...
        set {
            // By the nature of MGMarqueeLabel, this is false
...
public struct ScrollStep: MarqueeStep {
    /**
     ...
     The `away` position depends on the MGMarqueeLabel `type` value.
...
fileprivate extension CAMediaTimingFunction {
    ...
    func solveTforY(_ y_0: CGFloat, epsilon: CGFloat, controlPoints: [CGPoint]) -> CGFloat {
        ...
        print("MGMarqueeLabel: Failed to find t for Y input!")
        return t0
    }
